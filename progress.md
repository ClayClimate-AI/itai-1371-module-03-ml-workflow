...
- **Deliverable 1:** `L03_SingleEpoch_ITAI1371.pdf` — 23 pages, one continuous kernel restart →
  ...
  export attempt clipped long code lines; caught by the visual audit, refined per ADR 0006's
  amendment, re-verified page by page.
- **Deliverable 2:** `L03Journal_R_SingleEpoch_ITAI1371.pdf` — reflective journal, 2 pp, including
  the corrected Decision-Tree analysis (it did NOT overfit per Cell 16; the accuracy gap is
  decision-boundary shape, not memorization).
- **Deliverable 3:** `L03Journal_C_SingleEpoch_ITAI1371.pdf` — solo contribution journal, plain
  first-person terms, leads with the Learning-Curve addition as the standout piece of extra work.
- **C4 transcript:** `reflections.md` — 4 verbatim answers (comprehension, verification methods,
  contract drift, local-vs-objective proof); territory novelty omitted at Pilot's discretion, noted
  not silently dropped.
- **README:** Results section with the real Learning-Curve PNG (`assets/learning_curves.png`) and
...
- **Next Single Action:** Confirm CI is green on `build/lab03-josephclay`'s latest commit, then
  open a PR into `main` per the collaborative Git workflow (ADR 0001).
